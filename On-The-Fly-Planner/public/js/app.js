...
function runFuelCalc(forceRender = false) {
    ...
    waypoints.forEach((wp, index) => {
        ...
        // *** FIX: PREVENT NEGATIVE FUEL ***
        if (currentFuel < 0) currentFuel = 0;

        wp.calculatedFuel = Math.round(currentFuel);

        // *** FIX: LIVE COLOR UPDATE ***
        const fuelCell = document.getElementById(`val-fuel-${index}`);
        if (fuelCell) {
            fuelCell.innerText = wp.calculatedFuel;
            
            // Remove old classes
            fuelCell.classList.remove('fuel-good', 'fuel-joker', 'fuel-bingo');

            // Apply new class
            if (currentFuel <= fuelState.bingo) {
                fuelCell.classList.add('fuel-bingo'); // RED
            } else if (currentFuel < fuelState.joker) {
                fuelCell.classList.add('fuel-joker'); // YELLOW
            } else {
                fuelCell.classList.add('fuel-good'); // GREEN
            }
        }
    ...
    if(landingEl) {
        ...
        // Landing Fuel Colors
        landingEl.style.color = '#2ecc71'; // Default Green
        if (currentFuel <= fuelState.bingo) landingEl.style.color = '#e74c3c'; // Red
        else if (currentFuel < fuelState.joker) landingEl.style.color = '#f1c40f'; // Yellow
    }
...
function renderTable(force = false) {
    ...
    waypoints.forEach((wp, index) => {
        ...
        let tgtElevCell = (wp.data.type === 'TGT') ? 
            // Added class 'wp-tgt-input' below
        ...
        // TAS Display (Now with ID for live updates)
        // We default to empty if not computed yet
        const tasVal = (wp.computed && wp.computed.tas) ? `/${wp.computed.tas}T` : "";
        const tasDisplay = `<span id="val-tas-${index}" class="tas-display">${tasVal}</span>`;
...
// 1. Identify Threat Type & Range
function identifyThreat(u) {
    // Filter: Hostile + Ground/Sea
    ...
    for (const [key, data] of Object.entries(SAM_DB)) {
        if (nameUpper.includes(key)) {
            type = data.type;
            ceil = data.ceil;
            if (type === "LORAD") rangeNM = 60;
            else if (type === "MERAD") rangeNM = 25;
            else if (type === "SHORAD") rangeNM = 8;
            else if (type === "AAA") rangeNM = 3;
            return { type, ceil, rangeNM, name: rawName };
        }
    }

    // B. Fallback SAM
    if (nameUpper.includes("SAM") || nameUpper.includes("RADAR")) {
        return { type: "SHORAD", ceil: 20000, rangeNM: 8, name: rawName };
    }
    ...
    if (isArmor) {
        return { type: "ARMOR", ceil: 5000, rangeNM: 3, name: rawName };
    }

    // D. Naval Check
    if (u.c === 4) {
        // SUPER CAPITAL SHIPS (S-300F Equipped - Deadly)
        // Updated to catch "PIOTR" specifically
        if (nameUpper.includes("MOSKVA") || nameUpper.includes("PIOTR") || nameUpper.includes("PYOTR") || nameUpper.includes("KIROV") || nameUpper.includes("SLAVA")) {
            // Treat like an S-300 (60nm range, infinite ceiling)
            // LORAD classification ensures it shows up Red and high priority
            return { type: "LORAD", ceil: 99000, rangeNM: 60, name: rawName };
        }
        
        // STANDARD CAPITAL SHIPS (CVN, LHA, Ticonderoga, etc)
        else if (nameUpper.includes("CVN") || nameUpper.includes("CARRIER") || nameUpper.includes("CRUISER") || nameUpper.includes("TICONDEROGA")) {
            return { type: "NAVAL", ceil: 60000, rangeNM: 40, name: rawName };
        } 
        
        // SMALL BOATS
        else if (nameUpper.includes("SPEEDBOAT") || nameUpper.includes("BOAT")) {
            return { type: "NAVAL", ceil: 5000, rangeNM: 3, name: rawName };
        }
        
        // FRIGATES / CORVETTES / CARGO
        else {
            return { type: "NAVAL", ceil: 25000, rangeNM: 15, name: rawName };
        }
    }
...
// 3. Draw/Update Ring Helper
function updateMarkerRing(marker) {
    const u = marker.unitData;
    
    // Remove existing if turned off or no longer valid
    if (!showThreatRings) {
        if (marker.threatCircle) {
            map.removeLayer(marker.threatCircle);
            marker.threatCircle = null;
        }
        return;
    }
    ...
    if (threatInfo) {
        const radiusMeters = threatInfo.rangeNM * 1852;
        ...
        const color = colors[threatInfo.type] || "#e74c3c";

        if (marker.threatCircle) {
            // Update position
            marker.threatCircle.setLatLng(marker.getLatLng());
            marker.threatCircle.setRadius(radiusMeters);
        } else {
            // Create new
            marker.threatCircle = L.circle(marker.getLatLng(), {
                color: color,
                fill: false,
                // fillOpacity: 0.05,
                weight: 1,
                radius: radiusMeters,
                interactive: false // Allow clicking through to the unit
            }).addTo(map);
        }
    } else {
        // Clean up if unit type changed (unlikely but safe)
        if (marker.threatCircle) {
            map.removeLayer(marker.threatCircle);
            marker.threatCircle = null;
        }
