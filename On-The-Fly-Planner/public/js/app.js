...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- MDC (TEXT) MODE ---
    if (mode === 'mdc') {
        document.body.classList.add('mdc-mode');
        document.getElementById('dashboard-panel').style.display = 'block';
        
        // 1. ISOLATE THE KNEEBOARD
        const items = document.querySelectorAll('.accordion-item');
        
        items.forEach(item => {
            const hasMDC = item.querySelector('#kneeboard-container');
            
            if (hasMDC) {
                // Hide Header, Show Body
        ...
        // 2. Render Data
        setTimeout(renderKneeboard, 500);
    }
    ...
    // --- FUEL CONTROL (WEAPONS HEADER) ---
    const fuelInput = document.getElementById('total-fuel-input');
    const fuelSlider = document.getElementById('fuel-slider');

    // Sync Slider -> Input (Live calc, no broadcast yet)
    fuelSlider.addEventListener('input', (e) => {
        fuelInput.value = e.target.value;
        runFuelCalc(); 
    });

    // Sync Input -> Slider & Broadcast (On Release/Commit)
    fuelSlider.addEventListener('change', () => {
        saveData();
        broadcastRouteState();
    });

    // Sync Input Box -> Slider & Broadcast
    ...
    // --- NAV DASHBOARD LISTENERS (UPDATED TO BROADCAST) ---
    // These now save and broadcast so the Web App sees the fuel changes instantly
    document.getElementById('taxi-fuel').addEventListener('change', () => { runFuelCalc(); saveData(); broadcastRouteState(); });
...
document.getElementById('cv-depart-check').addEventListener('change', () => { runFuelCalc(); saveData(); broadcastRouteState(); });
    ...
    // 1. Manual Notes Sync
    const mdcNotes = document.getElementById('mdc-notes');
    if(mdcNotes) {
        mdcNotes.addEventListener('change', () => {
            saveData();
            broadcastRouteState();
        });
    }

    // 2. Countermeasures Sync
    document.getElementById('cm-chaff').addEventListener('change', () => { updateCM('chaff'); saveData(); broadcastRouteState(); });
    document.getElementById('cm-flare').addEventListener('change', () => { updateCM('flare'); saveData(); broadcastRouteState(); });
...
function setupLoadoutListeners() {
    for(let i=1; i<=9; i++) {
        const el = document.getElementById(`stn-${i}`);
        if(el) {
            // Update: Run Calc, Save, AND BROADCAST
            el.addEventListener('change', () => {
                runFuelCalc();
                saveData(); 
                broadcastRouteState(); // <--- ADD THIS
            });
        }
    }
}
...
function checkSetupState() {
    // 1. BYPASS IF IN KNEEBOARD OR MDC MODE
    // We do not want popups blocking the view in OpenKneeboard
    if (document.body.classList.contains('kb-mode') || document.body.classList.contains('mdc-mode')) {
        return; 
    }
...
socket.on('externalRouteUpdate', (payload) => {
    ...
    if (payload.waypoints) {
        payload.waypoints.forEach(rawWp => reconstructWaypoint(rawWp));
    }
    if (payload.history) {
    ...
    // 2. Apply Loadout
    if (payload.loadout) {
        for (const [id, val] of Object.entries(payload.loadout)) {
            const el = document.getElementById(id);
            if(el) el.value = val;
        }
    }

    // 3. Apply Fuel & Settings
    if (payload.fuel) {
        if(payload.fuel.start) document.getElementById('total-fuel-input').value = payload.fuel.start;
        if(payload.fuel.taxi) document.getElementById('taxi-fuel').value = payload.fuel.taxi;
        if(payload.fuel.joker) document.getElementById('joker-fuel').value = payload.fuel.joker;
        if(payload.fuel.bingo) document.getElementById('bingo-fuel').value = payload.fuel.bingo;
    ...
    // 4. Apply Mech (New)
    if (payload.mech) {
        if(payload.mech.chaff) document.getElementById('cm-chaff').value = payload.mech.chaff;
        if(payload.mech.flare) document.getElementById('cm-flare').value = payload.mech.flare;
        if(payload.mech.gun) document.getElementById('gun-rounds').innerText = payload.mech.gun;
        if(payload.mech.gunType) document.getElementById('gun-type').innerText = payload.mech.gunType;
        
        updateCM(); // Updates totals
    }

    // 5. Apply Notes (New)
    if (payload.notes !== undefined) {
        const notesEl = document.getElementById('mdc-notes');
        if(notesEl) notesEl.value = payload.notes;
    }

    // 6. RECALCULATE EVERYTHING
    // This updates the T/O Weight, Dist, V1/V2 based on the values we just injected
    calculateAircraftState(); 
    runFuelCalc();            
    updatePolyline();
    renderTable();
    ...
    setTimeout(() => {
        if (typeof refreshNavAccordionHeight === 'function') refreshNavAccordionHeight();
    }, 50);
...
function broadcastRouteState() {
    ...
    // 2. Capture Loadout State
    const loadoutState = {};
    for(let i=1; i<=9; i++) {
        const el = document.getElementById(`stn-${i}`);
        if(el) loadoutState[`stn-${i}`] = el.value;
    }

    // 3. Capture Fuel/Settings (Includes Bingo/Joker)
    const fuelSettings = {
        start: document.getElementById('total-fuel-input').value,
        taxi: document.getElementById('taxi-fuel').value,
        joker: document.getElementById('joker-fuel').value,
        bingo: document.getElementById('bingo-fuel').value,
        depart: document.getElementById('depart-time').value,
	cv: document.getElementById('cv-depart-check').checked
    };

    // 4. Capture Mech (New)
    const mechSettings = {
        chaff: document.getElementById('cm-chaff').value,
        flare: document.getElementById('cm-flare').value,
        gun: document.getElementById('gun-rounds').innerText,
        gunType: document.getElementById('gun-type').innerText
    };

    // 5. Capture Notes (New)
    // We use the class selector because IDs might vary in some views, but class is consistent
    const notesEl = document.getElementById('mdc-notes');
    const notesContent = notesEl ? notesEl.value : "";
    ...
    const payload = {
        ...
        fuel: fuelSettings,
        mech: mechSettings, // <--- ADDED
        notes: notesContent // <--- ADDED
    };
...
// ==========================================
// AUTO-ENTRY (UFC PUNCHER) - HTML MODAL VERSION
// ==========================================
...
function autoEntryWaypoints() {
    ...
    // 2. Validation
    if (waypoints.length < 2) {
        // We can use a standard alert here if needed, or just silent return in KB mode
    ...
    // 4. SHOW HTML MODAL (Visible in OpenKneeboard)
    const checklistHTML = `
    ...
    // Use our custom modal system instead of window.confirm
    showConfirm(checklistHTML, executePunch);
}
