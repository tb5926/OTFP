...
const DEFAULT_COMMS = {
    c1: [
        {n:"MANUAL", f:"305.000"}, {n:"TWR 1", f:"264.000"}, {n:"TWR 2", f:"265.000"}, {n:"GND", f:"250.000"}, 
        {n:"APP", f:"251.000"}, {n:"DEP", f:"252.000"}, {n:"AWACS", f:"253.000"}, {n:"JSTARS", f:"254.000"}, 
        {n:"TANK 1", f:"255.000"}, {n:"TANK 2", f:"256.000"}, {n:"TAC 1", f:"257.000"}, {n:"TAC 2", f:"258.000"},
        {n:"SQDN", f:"259.000"}, {n:"AUX 1", f:"260.000"}, {n:"AUX 2", f:"261.000"}, {n:"AUX 3", f:"262.000"},
        {n:"TEST", f:"263.000"}, {n:"SAR", f:"264.000"}, {n:"GUARD", f:"243.000"}, {n:"EMPTY", f:"---.---"}
    ],
    c2: [
        {n:"MANUAL", f:"127.000"}, {n:"CV TWR", f:"127.500"}, {n:"CV APP", f:"128.000"}, {n:"CV MAR", f:"129.000"},
        {n:"LSO", f:"130.000"}, {n:"JTAC 1", f:"131.000"}, {n:"JTAC 2", f:"132.000"}, {n:"FAC", f:"133.000"},
        {n:"FLIGHT", f:"134.000"}, {n:"WING", f:"135.000"}, {n:"CONVOY", f:"136.000"}, {n:"ARTY", f:"137.000"},
        {n:"RANGE", f:"138.000"}, {n:"FARRP", f:"139.000"}, {n:"UNICOM", f:"140.000"}, {n:"ATIS", f:"141.000"},
        {n:"FSS", f:"142.000"}, {n:"SAR", f:"143.000"}, {n:"GUARD", f:"121.500"}, {n:"EMPTY", f:"---.---"}
    ]
};
...
function runFuelCalc(forceRender = false) {
    ...
    waypoints.forEach((wp, index) => {
        ...
        if (index > 0) {
            ...
            // 1. Get User Indicated Speed (IAS)
            let indicatedSpeed = parseFloat(wp.data.spd) || 350;
            
            // 2. Calculate True Airspeed (TAS)
            ...
            // 4. Calculate Ground Speed (Using TAS)
            ...
            // 6. Fuel Penalty Logic (Using TAS for Mach Calc)
            ...
            // For WP0, we just show the TAS for the altitude/input, even if not moving
            let legAlt = parseFloat(wp.data.alt) || 0;
        ...
        wp.computed = {
            ...
            gs: groundSpeed,
            tas: Math.round(trueAirSpeed), // Store TAS for table
    ...
    if (fuelCell) {
        ...
        // Only change class if needed (prevents flicker)
        if (fuelCell.className !== fClass) {
            fuelCell.className = fClass;
        }
    }
    ...
    if(landingEl) {
        ...
        landingEl.style.color = currentFuel < fuelState.bingo ? '#e74c3c' : (currentFuel < fuelState.joker ? '#f1c40f' : '#2ecc71');
    }
...
function initThreatFilterMenu() {
    ...
    container.innerHTML = '';
    // Added "ARMOR" to this list
    const types = ["LORAD", "MERAD", "SHORAD", "AAA", "ARMOR"];
...
function initMap() {
    ...
    const MapToolsControl = L.Control.extend({
        ...
        onAdd: function(map) {
            ...
            // BUTTON 1: RESET / TRASH
            const btnReset = L.DomUtil.create('button', '', container);
            btnReset.innerHTML = '🗑️'; 
            btnReset.title = "Clear All Waypoints";
            stop(btnReset); 
            btnReset.onclick = (e) => { e.preventDefault(); resetNavigation(e); };

            // BUTTON 2: UNDO
            const btnUndo = L.DomUtil.create('button', '', container);
            btnUndo.innerHTML = '↩️'; 
            btnUndo.title = "Undo Last Waypoint";
            btnUndo.style.fontSize = "18px"; 
            stop(btnUndo); 
            btnUndo.onclick = (e) => { e.preventDefault(); undoLastWaypoint(); };

            // 2. NAV TOOLS (< >)
            const navRow = L.DomUtil.create('div', 'nav-row', container);
            const btnPrev = L.DomUtil.create('button', 'nav-btn-half', navRow);
            btnPrev.innerHTML = '◄'; stop(btnPrev); 
            btnPrev.onclick = (e) => { e.preventDefault(); cycleMapFocus(-1); };
            
            const btnNext = L.DomUtil.create('button', 'nav-btn-half', navRow);
            btnNext.innerHTML = '►'; stop(btnNext); 
            btnNext.onclick = (e) => { e.preventDefault(); cycleMapFocus(1); };
...
function renderTable(force = false) {
    ...
    waypoints.forEach((wp, index) => {
        ...
        let tgtElevCell = (wp.data.type === 'TGT') ? 
            `<input type="number" class="wp-input" style="color:var(--danger-color); border-bottom:1px solid #333;" 
        ...
        // *** UPDATED: Added ID for live updates ***
        ...
        let onloadCell = (wp.data.type === 'TANKER') ?
            `<input type="number" class="onload-input" 
                    ...
                    onchange="saveData()">` : // Save on commit
        ...
        const tasDisplay = wp.computed && wp.computed.tas ? 
            `<span class="tas-display">/${wp.computed.tas}T</span>` : "";
        ...
        tr.innerHTML = `
            ...
            <td><input type="number" class="wp-input wp-alt-input" value="${wp.data.alt}" ${noDrag} onchange="updateWpData(${index}, 'alt', this.value)"></td>
            <td>${tgtElevCell}</td>
        ...
        if (wp.uiExpanded && wp.computed) {
            ...
            // Coordinates logic... (Keep existing)
            ...
            primaryCoord = coordStr.replace(/([NSEW])\s+/g, '$1');
                ...
                <td colspan="10">
                    <div class="detail-content-wrapper">
                        ...
                        <div class="detail-group">
                            ...
                            <span id="val-ete-${index}" class="detail-val">${eteStr}</span>
                        </div>
                        <div class="detail-group">
                            <label>Ground Spd</label>
                            <span id="val-gs-${index}" class="detail-val">${gsStr}</span>
                        </div>
                        ...
                        <div class="detail-group">
                            <label style="color:var(--accent-color)">ETA (Local)</label>
                            <span id="val-eta-${index}" class="detail-val" style="color:var(--accent-color)">${etaStr}</span>
                        </div>
                    </div>
                </td>`;
...
function openModal() {
    document.getElementById('wp-name').value = `WP ${waypoints.length + 1}`;
    document.getElementById('wp-type').value = 'WP';
    document.getElementById('wp-onload').value = 0;
    checkWpType(); 
    modal.style.display = 'flex';
    fetchGroundElev('wp-tgt-alt');
}
...
function setupAccordions() {
    document.querySelectorAll('.accordion-header').forEach(acc => {
        acc.addEventListener('click', function(e) {
            if (e.target.classList.contains('btn-reset-section')) return;
            ...
            if (panel.style.maxHeight) {
                // --- CLOSING ---
                // 1. Immediately hide overflow so it slides up cleanly
                ...
                // 3. DROPDOWN FIX: Allow overflow after animation (300ms)
                setTimeout(() => {
...
function analyzeThreats() {
    ...
    waypoints.forEach((wp, idx) => {
        ...
        for (const [id, unitMarker] of Object.entries(liveMarkers)) {
            ...
            const isGround = (u.c === 3 || u.c === 4);
            if (!isEnemy || !isGround) continue;
            ...
            if (!threatType) {
                const isArmor = nameUpper.includes("TANK") || nameUpper.includes("T-") || 
                                ...
                                nameUpper.includes("M113") || 
                                nameUpper.includes("SAU") || 
                                nameUpper.includes("GEPARD") || 
                                nameUpper.includes("MTLB") || 
                                nameUpper.includes("BRADLEY") || nameUpper.includes("M2") || nameUpper.includes("M3") ||
                                nameUpper.includes("WARRIOR") ||
                                nameUpper.includes("LECLERC") || nameUpper.includes("MERKAVA") ||
                                nameUpper.includes("TOS") || 
                                nameUpper.includes("SCUD") || 
                                nameUpper.includes("AAV");
    ...
    if (uniqueThreats.size === 0) {
        ...
        const typeOrder = ["LORAD", "MERAD", "SHORAD", "AAA", "ARMOR"];
        const colors = { "LORAD": "#ff5555", "MERAD": "#e74c3c", "SHORAD": "#e67e22", "AAA": "#f1c40f", "ARMOR": "#d35400" };
        ...
        typeOrder.forEach(type => {
            ...
            if (list.length > 0) {
                ...
                // --- GRID ROWS ---
                list.forEach(t => {
                    // 1. Primary Coordinate (Based on Settings)
                    let coordStr = "";
                    if (typeof CoordConverter !== 'undefined') {
                    ...
                    // Compact format (remove extra spaces/commas)
                    coordStr = coordStr.replace(/([NSEW])\s+/g, '$1').replace(',', '');
                    
                    // 2. Secondary Coordinate (Always MGRS 10-digit)
...
function getUnitIcon(unit, color, isOwnship = false) {
    ...
    // --- 0. ORDNANCE (White Dots) ---
    if (!isOwnship && (name.includes("MISSILE") || name.includes("ROCKET") || name.includes("SHELL") || name.includes("BOMB") || name.includes("AIM-") || name.includes("AGM-"))) {
        return L.divIcon({
            className: 'unit-marker',
            html: `<div id="icon-shape-${unitId}" style="width:12px; height:12px;">
                     <svg width="12" height="12" viewBox="0 0 12 12" style="filter: drop-shadow(1px 1px 2px black);">
    ...
    // --- 3. GROUND / SHIP / SAM ---
        ...
        if (name.includes("FARP")) {
            ...
            const isSAM =
                ...
                name.includes("IGLA") || name.includes("STRELA") || name.includes("HEMTT_C-RAM_PHALANX") || 
    ...
    const labelHtml = labelText ? 
        `<div style="position:absolute; top:0; left:0; width:24px; height:24px; display:flex; align-items:center; justify-content:center; pointer-events:none;">
            <span style="font-family:Arial; font-weight:900; font-size:${labelSize}; color:white; text-shadow:1px 1px 2px black;">${labelText}</span>
         </div>` : '';
...
function autoEntryWaypoints() {
    ...
    // (Optional: Check if socket is connected, though emit usually just fails silently if not)
    
    // 3. Safety Checklist (From your debug.html)
    ...
    // Iterate (Skip Index 0 / Ownship)
    for (let i = 1; i < waypoints.length; i++) {
        const wp = waypoints[i];
        const latlng = wp.marker.getLatLng();

        // Elevation Logic (Target vs Cruise)
        ...
        punchList.push({
            ...
            name: wp.data.name,
            lat: latlng.lat,
            long: latlng.lng, // *** IMPORTANT: Server expects 'long', not 'lng' ***
    ...
    if(statusEl) {
        statusEl.innerText = "Data Transmitting..."; // Set initial text
        statusEl.style.color = "#f1c40f";
    }

    // 6. Send to Node Server
...
socket.on('punchStatus', (msg) => {
    ...
    // You can add a span with id="punch-status" next to the button in HTML to see this
    const el = document.getElementById('punch-status');
    if(el) {
        el.innerText = msg;
        ...
        if(msg.includes("Complete")) {
            ...
            el.style.color = "#f1c40f"; // Yellow (Typing...)
        }
    }
});
